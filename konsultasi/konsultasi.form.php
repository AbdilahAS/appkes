        ...
        <div class="navbar-inner" style="border:1px solid #bbb; border-radius:10px; padding:10px 20px 10px 20px; margin-top:50px; margin-left:auto; margin-right:auto;">
			<div>
            	<div class="text-error input-prepend pull-left">
                	<h4>Checklist gejala yang dialami </h4>
                </div>
                ...
                <div style="margin-top:10px">
                    <form name="form1" method="post" action="">
                        <table class="table table-condensed table-hover">
                            <?php 
                                $sg = mysqli_query($connect,"select * from gejala");
                        ...
                        <?php
							if(isset($_POST['periksa'])){
								...
								$sk = mysqli_query($connect,"select * from hasil_konsultasi group by waktu desc limit 1");
								...
								$konsultasi=$k+1;
									...
									for($i=0; $i < $jumlah; $i++){
										...
										$sb = mysqli_query($connect,"select * from gejala where id_gejala=$gejala");
										$rb = mysqli_fetch_array($sb);
										$bobot = $rb['bobot'];
										
										$q=mysqli_query($connect,"insert into hasil_konsultasi(id_user, konsultasi, id_gejala, bobot, waktu) value ('$id', $konsultasi, '$gejala', '$bobot', NOW())") or die(mysql_error());
										
										if ($q){
											$nk = mysqli_query($connect,"select * from hasil_konsultasi group by waktu desc limit 1");
